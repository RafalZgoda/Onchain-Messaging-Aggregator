...
export default function Chat({
	...
	const handleSendMessage = async () => {
		...
		const isNativeActive = platformsFilter.includes(
			MESSAGE_PLATFORMS.native
		);

		await sendAggregatedMessage({
			conversation_xmtp:
				isXMTPActive && activeConversation.conversation_xmtp,
			message: inputValue,
			signer,
	...
	const getMessages = async () => {
		...
		const messages = await getAggregatedMessages({
			...
			conversation_push_request:
				activeConversation?.conversation_push_request,
					...
					<div className="absolute z-10 w-full bg-black/70 flex justify-center items-center">
						<div className="bg-white/10 w-[34rem] h-[24rem] flex-row">
							...
							<input
								...
								placeholder="Address"
								onChange={(e) =>
									setNewMessageAddress(e.target.value)
								}
													...
													(platform, index) => (
														...
														>
															<Checkbox
																...
																checked={platformsFilter.includes(
																	platform
																)}
																className="w-3 h-3 mr-5"
															/>
